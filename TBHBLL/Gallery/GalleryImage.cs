...
namespace BBICMS
{
    public class GalleryImage
    {
        ...
        public void MakeThumbnail(ref string sSource, string sDestination, ref int MaxWidth, ref int MaxHeight,
                                  InterpolationMode nMode, [Optional, DefaultParameterValue(100)] int iJPGQuality)
        {
            double dblCoef;
            int iHeight;
            int iWidth;
            ...
            {
                objBitmap.Save(sDestination, GetEncoder(ImageFormat.Gif), objEncoder);
            }
            else if (Path.GetExtension(sDestination) == ".jpg")
            {
                objBitmap.Save(sDestination, GetEncoder(ImageFormat.Jpeg), objEncoder);
            }
            else if (Path.GetExtension(sDestination) == ".png")
            {
                objBitmap.Save(sDestination, GetEncoder(ImageFormat.Png), objEncoder);
            }
            objBitmap.Dispose();
            objImage.Dispose();
        }

        public string StoreImage(string sDestinationPath, string sDestinationFileName, string ImgFile, int EleWidth,
                                 int EleHeight)
        {
            return StoreImage(sDestinationPath, sDestinationFileName, ImgFile, EleWidth, EleHeight, string.Empty);
        }

        public string StoreImage(string DestinationPath, string DestinationFileName, string ImageFileName,
                                 int ImageWidth, int ImageHeight, string CurrentImageFileName)
        {
            return StoreImage(DestinationPath, DestinationFileName, ImageFileName, ImageWidth, ImageHeight,
                              CurrentImageFileName, 100);
        }

        public string StoreImage(string DestinationPath, string DestinationFileName, string ImageFileName,
                                 int ImageWidth, int ImageHeight, string CurrentImageFileName, int ImageQuality)
        {
            if (!string.IsNullOrEmpty(DestinationFileName))
            {
                DestinationFileName = Path.GetFileName(DestinationFileName).Replace("#", string.Empty);
                DestinationFileName = Path.Combine(DestinationPath, StoreImageExtracted(ref DestinationFileName));
                CheckFileExists(DestinationFileName);
                MakeThumbnail(ref ImageFileName, DestinationFileName, ref ImageWidth, ref ImageHeight,
                              InterpolationMode.HighQualityBicubic, ImageQuality);
