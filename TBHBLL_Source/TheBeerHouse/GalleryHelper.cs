namespace TheBeerHouse
{
    ...
    public class GalleryHelper
    {
        ...
        public static bool EnsureAlbumTreeExist(string AlbumName)
        {
            ...
            if ((!Directory.Exists(GetAlbumOriginalsDirectory(AlbumName)) | !Directory.Exists(GetAlbumDisplayDirectory(AlbumName))) | !Directory.Exists(GetAlbumThumbNailsDirectory(AlbumName)))
            {
                return false;
            }
            return true;
        ...
        {
            return Path.Combine(Helpers.WebRoot, Path.Combine(Helpers.Settings.Gallery.PhotosDirectory, Path.Combine(Helpers.FormatSpacesForURL(vAlbumName), Path.Combine(vPictureType, vFileName))));
        }
